...
def create_polars_df():
    pl.Config.set_streaming_chunk_size(4000000)
    return (
        pl.scan_csv("data/measurements.txt", separator=";", has_header=False, new_columns=["station", "measure"], schema={"station": pl.Utf8, "measure": pl.Float64})
        .group_by("station")
        .agg(
            max = pl.col("measure").max(),
            min = pl.col("measure").min(),
        ...
        .sort("station")
        .collect()  # Não use streaming=True aqui se não for necessário
    )
